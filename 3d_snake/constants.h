...
// ===== 窗口 =====
const int SCREEN_WIDTH = 2560; // 窗口宽度。
const int SCREEN_HEIGHT = 1440; // 窗口高度。

// ===== 地图和关卡 =====
const int DEFAULT_GRID_SIZE = 15; // 非法关卡时使用的默认地图边长。
const int LEVEL_COUNT = 4; // 菜单里一共有 4 个关卡按钮。

// ===== 菜单设置范围 =====
const int MIN_SPEED_LEVEL = 1; // 最低速度档。
const int MAX_SPEED_LEVEL = 5; // 最高速度档。
const int MIN_DIFFICULTY_LEVEL = 1; // 最低难度档。
const int MAX_DIFFICULTY_LEVEL = 5; // 最高难度档。
...
// ===== 文件路径 =====
const char *const HIGH_SCORE_FILE = "highscore.txt"; // 最高分文件路径。
const char *const MENU_BACKGROUND_FILE = "assets/menu_background.png"; // 背景图路径。

// ===== 3D 绘制 =====
const float CUBE_SIZE = 1.0f; // 一个地图格子对应的 3D 方块大小。
...
// ===== 主菜单布局 =====
const int MENU_WIDTH = 800; // 主菜单面板宽度。
const int MENU_HEIGHT = 750; // 主菜单面板高度。
const int MENU_Y = 150; // 主菜单面板顶部 y 坐标。
